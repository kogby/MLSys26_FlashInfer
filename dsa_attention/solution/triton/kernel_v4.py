"""
Sparse Attention Triton Kernel V4 — Always-KV-Split.

V3 result on B200:
  T<3 workloads (V2 direct path):  0.19ms, 6× speedup       <- bottleneck
  T≥3 workloads (KV-split path):   0.06ms, 30–36× speedup
V2 direct was slow because it launches 1 block per token → on B200's 160 SMs,
~99% of SMs idle. KV-split launches 8 blocks per token → 8× better utilization
even for T=1.

V4 change: T_SMALL_THRESHOLD lowered 3 → 1 so KV-split runs for T=1, 2. The
two-kernel launch overhead (~10μs) is dwarfed by the parallelism gain.

Everything else unchanged from V3:
  - Flash-Decoding split over TOPK=2048 into 8 chunks of 256
  - _compute_kernel: per-split attention, writes partial_out + (m, l)
  - _reduce_kernel:  LSE merge across splits
  - Workspace cached across calls
  - -1e30 sentinel (not -inf) avoids NaN in empty-split case
"""
...
T_SMALL_THRESHOLD = 1   # V4: always use KV-split (threshold=1 means T<1 never).
                         # V3 used 3; V4 test: does always-split help T=1,2?
                         # B200 has 160 SMs — 1 block/token leaves 99% idle.
